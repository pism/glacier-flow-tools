...
def test_linear_flow_np(create_linear_flow: xr.Dataset):
    ...
    pts, _, _, _ = compute_pathline(
        starting_point, velocity, f_args=(Vx, Vy, x, y), start_time=0, end_time=total_time, hmin=dt, hmax=dt, tol=1e-1
    )
    assert_array_almost_equal(pts[-1, :], *r_exact)

    pts, _, _, _ = compute_pathline(
        starting_point, velocity, f_args=(Vx, Vy, x, y), start_time=0, end_time=total_time, hmin=1e-6, hmax=dt, tol=1e-4
    )
    assert_array_almost_equal(pts[-1, :], *r_exact)
